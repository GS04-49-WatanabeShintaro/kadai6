...
//ファイルに書き込んでいく
$file = fopen("data/data.csv","a"); //決まり事。書き込む前にファイルオープン。
...
while ($array = fgetcsv( $fp )) { //カンマ区切りのcsvをarrayに入れて配列化
      $num = count($array); //配列の数を調べてnumに代入
      ...
      $yesPer = substr($yesko / ($num - 1) * 100, 0, 2);
      $noPer = 100 - $yesPer;
}
